  ...
  require(["jquery", "template", "cookie", "shseido"], function ($, template) {
    ...
    Cart.prototype = {
      ...
      load: function load() {
        ...
        $(".empty").hide().siblings(".not-empty").removeClass("ac"); //渲染模板
      ...
      addListener: function addListener() {
        // 删除
        $(".add_cart_info").on("click", ".del", $.proxy(this.delHandler, this)); // +/-数量

        $(".add_cart_info").on("click", ".decrement,.increment", $.proxy(this.decIncHandler, this)); // 输入修改数量

        $(".add_cart_info").on("blur", ".amount", $.proxy(this.inputHandler, this)); // 全选

        $(".allchecked").on("click", $.proxy(this.ckAllHandler, this)); // 部分选中

        $(".add_cart_info").on("click", ".cart_checked", $.proxy(this.ckProdHandler, this));
      },
      // 删除单行
      delHandler: function delHandler(e) {
        var target = e.target; // 获取事件源元素所在行

        var li = $(target).parents(".cart_li"); // 获取商品 id

        var id = li.find(".add_pro_img").attr("data"); // 从数组中删除该 id 对应的商品对象
      ...
      // +/-数量处理
      decIncHandler: function decIncHandler(e) {
        var target = e.target;
        var li = $(target).parents(".cart_li"); // 商品 id

        var id = li.find(".add_pro_img").attr("data"); // 商品对象
        ...
        $("#products_num").products_num(); // 页面渲染
      ...
      // 全选
      ckAllHandler: function ckAllHandler(e) {
      ...
      // 反选
      ckProdHandler: function ckProdHandler(e) {
        // 获取购物车主体中选中的复选框个数
        var count = $(".cart_checked:checked").length; // 设置全选复选框选中状态
